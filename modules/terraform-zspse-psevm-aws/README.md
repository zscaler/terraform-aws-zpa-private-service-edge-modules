...
| <a name="input_user_data"></a> [user\_data](#input\_user\_data) | Cloud init data (one entry per Service Edge VM). Each VM receives its own user\_data so per-instance values like the OAuth SSM parameter name can be injected. | `list(string)` | n/a | yes |
